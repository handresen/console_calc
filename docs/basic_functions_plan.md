...
Status on `basic_functions`: unary user-defined functions are now implemented.
...
- unary user-defined functions only
- syntax `name(param): expr`
...
- calling `f(expr)` substitutes or binds the argument into the stored body at evaluation time
...
1. Extend assignment parsing in `apps/` to recognize `name(param): expr`
   alongside `name: expr`.
2. Refactor runtime definition storage to represent typed definitions rather
   than only raw expression aliases.
3. Extend identifier and call expansion so unary user function calls are
   recognized in a token-aware way.
4. Add safe parameter substitution or binding for user-defined calls.
...
- multi-argument user functions like `f(x, y): x + y`
